...
class Desc implements Comparator<String>{
	...
	/*
	 	...
	 	=> 이 멧드가 양수를 반환하면 두 값의 순서가 바뀐다.(오름차순이 기본임)
	 	
	 	- 오름차순 정렬일 경우
	 	=> 앞의 값이 크면 양수, 같은면 0, 앞의 값이 작으면 음수를 반환하도록 한다.
	 	
	 	- String 객체에는 정렬을 위해서 compareTo()메서드가 구현되어 있는데 
	 	이 메서드의 반환값은 오름 차순에 맞게 반환되도록 구현되어 있다.
	 	(Wrapper클래스와 Date, File클래스에도 구현되어 있다.)
	 */
	
	@Override
	public int compare(String str1, String str2) {
	  //return str1.compareTo(str2)*1;//오름차순으로 정렬 하고 싶을 때
	  //return str1.compareTo(str2)*-1; 내림차순으로 정렬 하고 싶을 때
