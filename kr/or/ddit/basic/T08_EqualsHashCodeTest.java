...
public class T08_EqualsHashCodeTest {
	...
	/**
	 ...
	 해시 함수에 의해 얻어진 값은 해시 값, 해시 코드, 해시 체크섬 또는 간단하게 해시라고 한다.
	 ...
	 그래서 객체가 서로 같은지 여부를 결정하려면 두 메서드를 재정의 해야한다.
	 	...
	 	2. 두 객체가 같으면 equals()메서드를 호출했을 때 true를 반환해야 한다.
	 	     즉, 객체a, b가 같다면 a.equals(b)와 b.equals(a) 둘 다  true이여야 한다.
	 	3. 두 객체의 hashcode가 같다고 해서 두 객체가 반드시 같은 객체는 아니다.
	 	    하지만, 두 객체가 같으면 hashcode가 같아야 한다.
	 	4. equals()메서드를 override하면 반드시 hashCode()메서드로 override해야 한다.
	 	5. hashCode()는 기본적으로 Heap메모리에 있는 각 객체에 대한 메모리 주소 매핑정보를 기반으로 한 정수값을 반환한다.
	 	  ...
	 	  hashCode()메서드에 사용하는 '해싱 알고리즘'에서 서로 다른 객체에 대하여 같은
	...
	public static void main(String[] args) {
		...
		System.out.println("remove(p2) 성공 영부 : "+set.remove(p2));
